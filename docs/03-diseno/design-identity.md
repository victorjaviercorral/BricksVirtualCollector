...
- **Motion / animaciones (con intencion, no decoracion):** manejo directo con seguimiento 1:1 del puntero (arrastre de tokens), momentum + rubber-banding en carrusel horizontal (formulas de proyeccion de velocidad y resistencia progresiva en los bordes), curvas de easing propias (no las nativas de CSS), entrada con pequeno rebote (metafora fisica de "encastre"), `prefers-reduced-motion` respetado atenuando en vez de eliminar. Basado en la combinacion obligatoria de las skills `emil-design-eng` + `apple-design` (constitution D.12, ver CHANGELOG del framework v0.1.1).
